(function(){
  ...
  function setPref(k,v){ try{ localStorage.setItem(k, JSON.stringify(v)); }catch(e){} }
  function getPref(k, d){ try{ var v = JSON.parse(localStorage.getItem(k)||'null'); return (v===null?d:v); }catch(e){ return d; } }
  function applyPrefs(){
    var hc = getPref('pref_hc', false);
    var dark = getPref('pref_dark', false);
    html.classList.toggle('hc', !!hc);
    html.classList.toggle('dark', !!dark);
    var cBtn = document.getElementById('btn-contrast');
    var tBtn = document.getElementById('btn-theme');
    if(cBtn){ cBtn.setAttribute('aria-pressed', hc ? 'true' : 'false'); }
    if(tBtn){ tBtn.setAttribute('aria-pressed', dark ? 'true' : 'false'); }
  }
  function setupPrefButtons(){
    var cBtn = document.getElementById('btn-contrast');
    var tBtn = document.getElementById('btn-theme');
    if(cBtn){
      cBtn.addEventListener('click', function(){ setPref('pref_hc', !getPref('pref_hc', false)); applyPrefs(); });
      cBtn.addEventListener('keydown', function(e){ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); cBtn.click(); }});
    }
    if(tBtn){
      tBtn.addEventListener('click', function(){ setPref('pref_dark', !getPref('pref_dark', false)); applyPrefs(); });
      tBtn.addEventListener('keydown', function(e){ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); tBtn.click(); }});
    }
  }
  ...
  var Templates = {
    home: function(){ return `
      <section class="hero">
        ...
        <figure>
          <img src="assets/img/hero.svg" alt="Ilustração de voluntários e educadores" loading="lazy">
        </figure>
      </section>
    ...
    projetos: function(){
      ...
      ${ data.map(function(p){
          ...
          <article class="card mt-3">
            <img src="assets/img/${p.img}" alt="${p.title}" loading="lazy">
            <div class="card-body">
              <h2>${p.title}</h2>
  ...
  var hb = document.querySelector('label[for="nav-toggle"]');
  if(hb){ hb.addEventListener('keydown', function(e){ if(e.key==='Enter'||e.key===' '){ e.preventDefault(); hb.click(); }}); }

  window.addEventListener('hashchange', navigate);
  window.addEventListener('load', function(){ applyPrefs(); setupPrefButtons(); navigate(); });
})();